...
namespace igLibrary.Vfx
{
	public class igVfxRangedCurveMetaField : igMetaField
	{
		...
		public override uint GetSize(IG_CORE_PLATFORM platform)
		{
			if (igArkCore._game == igArkCore.EGame.EV_SkylandersTrapTeam)
			{
				// Consists of a float followed by an igMemoryRefMetaField, due to alignment it's equivalent to this
				return igMemoryRefMetaField._MetaField.GetSize(platform) + igAlchemyCore.GetPointerSize(platform);
			}
			else
			{
				return 0x54;
